...
--FROM  Students
--JOIN  Groups		ON [group]   =  group_id --присоединить/связать
...
--SELECT
--			[Направление обучения]  = direction_name,
--			[Количесвто групп]		= COUNT(DISTINCT group_id),
--			[Количество студентов]	= COUNT(stud_id)
--FROM		Students
--RIGHT JOIN		Groups		ON [group]	= group_id
--FULL JOIN		Directions	ON direction =	direction_id
...
-- LEFT OUTER JOIN;
-- RIGHT OUTER JOIN;
-- FULL OUTER JOIN;-- отбились путые группы без студентов

--SELECT
--		[Группа]				=	group_name,
--		[Количество студентов]	=	COUNT(stud_id),
--		[Напрваление обучения]	= direction_name
--FROM	Students
--RIGHT JOIN	Groups			ON[group] = group_id
--JOIN  Directions			ON direction=direction_id
...
SELECT 
			...
			[Количество пустых групп] = (SELECT COUNT(group_id)
